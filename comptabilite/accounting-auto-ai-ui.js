...
async function saveGeminiAccountingKey(){
  ...
  try{if(btn)btn.disabled=true;if(msg){msg.className='status';msg.textContent='Test réel de la clé : attente des 2 réponses Gemini…'}const r=await geminiConfigApi('POST',{action:'save_test',api_key:apiKey}),details=(r.models||[]).map(x=>`${x.model}: ${x.ok?'OK':'ÉCHEC'}`).join(' · ');if(r.active&&r.saved){input.value='';if(msg){msg.className='status ok';msg.textContent=`ACTIVE · clé ${r.key_count||'?'} / ${r.max_keys||10} enregistrée · ${details}`};await loadGeminiConfig()}else{if(msg){msg.className='status err';msg.textContent=`INACTIVE · clé non enregistrée. ${r.reason||''} ${details}`}}}catch(e){if(msg){msg.className='status err';msg.textContent='INACTIVE · '+(e?.message||String(e))}}finally{if(btn)btn.disabled=false}
}

async function loadGeminiConfig(){
  const box=$('geminiConfigState');if(!box)return;
  try{const c=await geminiConfigApi('GET'),count=Number(c.key_count||0),max=Number(c.max_keys||10);if(c.last_test?.active&&count){box.className='status ok';box.textContent=`ACTIVE · ${count}/${max} clé(s) Gemini enregistrée(s). Bascule automatique entre les clés.`}else if(c.configured){box.className='status';box.textContent=`${count}/${max} clé(s) enregistrée(s) · test Gemini nécessaire.`}else{box.className='status err';box.textContent=`INACTIVE · 0/${max} clé Gemini enregistrée.`}}catch(e){box.className='status err';box.textContent='INACTIVE · '+(e?.message||String(e))}
}
