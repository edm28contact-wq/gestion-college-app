...
const boot=()=>{
  ...
  const MAX_PAGES=30;
  const DEFAULTS={nativePdfText:true,localOcr:true,fullScale:4.2,zoneScale:8.3,targetedReread:true};
  const SETTINGS_URL='https://zreegtzfpwrjgdhhunxx.supabase.co/functions/v1/accounting-pdf-verifier?action=status';
  let runtime={...DEFAULTS},runtimeLoadedAt=0;
  async function runtimeSettings(){
    if(Date.now()-runtimeLoadedAt<60000)return runtime;
    try{
      const r=await fetch(SETTINGS_URL,{cache:'no-store'}),j=await r.json();
      if(r.ok&&j?.reader_runtime)runtime={
        nativePdfText:j.reader_runtime.native_pdf_text_enabled!==false,
        localOcr:j.reader_runtime.local_ocr_enabled!==false,
        fullScale:Number(j.reader_runtime.ocr_full_scale||DEFAULTS.fullScale),
        zoneScale:Number(j.reader_runtime.ocr_zone_scale||DEFAULTS.zoneScale),
        targetedReread:j.reader_runtime.targeted_reread_enabled!==false
      };
    }catch{}
    runtimeLoadedAt=Date.now();return runtime
  }
  ...
  async function highQualityTextPdf(file){
    if(!file)throw new Error('PDF manquant.');await runtimeSettings();const bytes=new Uint8Array(await file.arrayBuffer()),pdf=await window.pdfjsLib.getDocument({data:bytes}).promise,count=Math.min(pdf.numPages,MAX_PAGES),pages=[];let totalNative=0,weak=0;
    for(let n=1;n<=count;n++){const page=await pdf.getPage(n),tc=runtime.nativePdfText?await page.getTextContent():{items:[]},vp=page.getViewport({scale:1}),native=clean((tc.items||[]).map(x=>String(x.str||'')).join(' ')),chars=alnum(native),items=(tc.items||[]).length,needsOcr=!runtime.nativePdfText||chars<MIN_NATIVE_CHARS||items<MIN_NATIVE_ITEMS;if(needsOcr)weak++;totalNative+=chars;pages.push({page,n,native,chars,items,needsOcr,nativeRows:nativeMeta(tc,n,vp)});notify(5+Math.round(24*n/count),`Analyse structure page ${n}/${count}…`)}
    const scanLike=!runtime.nativePdfText||totalNative<Math.max(1000,count*300)||weak>=Math.ceil(count/3),targets=runtime.localOcr?(scanLike?pages:pages.filter(x=>x.needsOcr)):[];
    if(targets.length){const worker=await makeWorker();for(let i=0;i<targets.length;i++){const x=targets[i],full=await fullPass(worker,x.page,x.n);x.ocr=full.text;x.ocrConfidence=full.confidence;x.ocrScale=full.scale;x.ocrMode=full.mode;if(runtime.targetedReread&&(full.confidence<93||x.needsOcr||full.text.length<600))x.zones=await zonePass(worker,x.page,x.n);notify(48+Math.round(45*(i+1)/targets.length),`OCR page ${x.n}/${count} · confiance ${Math.round(full.confidence)} %`)}await worker.terminate()}
    ...
    const layout=layoutFrom(pages,count);window.__lastPdfOcrMeta={version:4,file_name:file.name,page_count:count,scan_like:scanLike,settings:{...runtime},layout,pages:pages.map(x=>({page:x.n,native_chars:x.chars,native_items:x.items,ocr_confidence:x.ocrConfidence??null,ocr_mode:x.ocrMode??null,zones:(x.zones||[]).map(z=>({name:z.name,confidence:Math.round(z.confidence),scale:Number(z.scale.toFixed(2))}))}))};notify(97,targets.length?'Lecture adaptative haute définition terminée.':'Texte PDF natif exploitable.');return blocks.join('\n\n').trim()
  }
  window.textPdf=highQualityTextPdf;window.getLastPdfOcrMeta=()=>window.__lastPdfOcrMeta||null;window.__pdfOcrHq={active:true,version:4,get settings(){return {...runtime}},maxPages:MAX_PAGES};
