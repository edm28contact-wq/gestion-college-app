(()=>{
  ...
  function routeSandbox(raw){
    ...
    if(slug==='system-mode'||slug==='sandbox-router'||slug==='sandbox-api'||slug==='secretariat-account-api')return null;
  ...
  window.fetch=async function(input,init={}){
    ...
    if(state.active){
      ...
      if(routed){
        const next=typeof input==='string'?routed:new Request(routed,input);
      ...
      if(!['GET','HEAD','OPTIONS'].includes(method)&&s.includes('zreegtzfpwrjgdhhunxx.supabase.co/functions/v1/')){
        const payload={error:'BAC À SABLE ACTIF : cette opération n’a pas encore de route de test et la production a été bloquée par sécurité.',test_mode:true};
