(()=>{
  ...
  function banner(){
    ...
    el.textContent='MODE TEST ACTIF — aucune donnée réelle ne sera modifiée'+(state.reason?' — '+state.reason:'');
  ...
  window.fetch=async function(input,init={}){
    ...
    if(state.active&&!['GET','HEAD','OPTIONS'].includes(method)&&!isAllowedWrite(url)){
      const payload={error:'MODE TEST ACTIF : simulation sécurisée, aucune donnée réelle n’a été modifiée.',test_mode:true,reason:state.reason||null};
  ...
  async function refreshMode(){
    try{const r=await originalFetch(MODE_API+'?action=status',{cache:'no-store'}),j=await r.json();state.active=!!j.test_mode;state.reason=j.reason||'';state.started_at=j.started_at||null;state.loaded=true;banner();window.dispatchEvent(new CustomEvent('gestion-college-test-mode',{detail:{...state}}))}catch(e){console.warn('Mode test indisponible',e)}
  }
