(()=>{
  ...
  function banner(){
    ...
    el.textContent='MODE TEST ACTIF — aucune donnée réelle ne sera modifiée';
  ...
  window.fetch=async function(input,init={}){
    ...
    if(state.active&&!['GET','HEAD','OPTIONS'].includes(method)&&!isAllowedWrite(url)){
      const payload={error:'MODE TEST ACTIF : simulation sécurisée, aucune donnée réelle n’a été modifiée.',test_mode:true};
